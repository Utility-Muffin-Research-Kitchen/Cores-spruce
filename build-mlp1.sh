...
--flycast-fast-umrk-profile-gen is a maintainer-only PGO generation build. It
stages an instrumented flycast_fast_umrk core under
  $OUTPUT_DIR/profile-gen/
for device training. It reads and writes no core cache entry and never touches
the canonical build report, so a training run cannot ship an instrumented core.
EOF
}
...
set_cache_args() {
    CACHE_ARGS=(
        ...
        --profile-dir "$REPO_ROOT/profiles/mlp1"
        --info-source-dir "$FLYCAST_FAST_UMRK_INFO_DIR"
        --recipe-root "$REPO_ROOT"
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --stock-parity)
            ...
            ;;
        --flycast-fast-umrk-profile-gen)
            set_build_mode flycast-profile-gen
            ;;
...
case "$build_mode" in
    ...
    spruce-buildable)
        ...
        ;;
    flycast-profile-gen)
        requested_cores=(flycast_fast_umrk)
        # A training build keeps its own artifacts, report and cache: it must
        # never overwrite the canonical report or seed a core cache entry.
        REPORT_PATH="$OUTPUT_DIR/profile-gen/flycast_fast_umrk-generation.txt"
        REPORT_JSON_PATH="$OUTPUT_DIR/profile-gen/flycast_fast_umrk-generation.json"
        ;;
    explicit)
...
copy_core_info() {
    ...
    # Repo-owned info files win: libretro-super's info directory only knows the
    # cores it builds itself and never carries flycast_fast_umrk_libretro.info.
    if [[ -f "$FLYCAST_FAST_UMRK_INFO_DIR/$info_file" ]]; then
        source_info="$FLYCAST_FAST_UMRK_INFO_DIR/$info_file"
    elif [[ -f "$LIBRETRO_SUPER_SRC_DIR/dist/info/$info_file" ]]; then
...
apply_mlp1_core_patch() {
    local core="$1"
    # Dedicated lanes keep their own checkout, so the caller can name it; the
    # generic lanes keep resolving through the libretro-super source root.
    local src_dir="${2:-$LIBRETRO_SUPER_SRC_DIR/libretro-$core}"
...
verify_a55_compile_log() {
    ...
    read -r compile_count mcpu_count mtune_count a53_count < <(
        awk '
            # The toolchain may invoke the compiler by absolute path
            # (/opt/mlp1-toolchain/bin/...), so match the basename anywhere.
    ...
    if [[ "$compile_count" -eq 0 || "$mcpu_count" -ne "$compile_count" \
        ...
        echo "$core did not apply both Cortex-A55 flags to every captured compile command" \
            "(commands=$compile_count mcpu=$mcpu_count mtune=$mtune_count)" >&2
        return 1
    fi
    ...
    case "$core" in
        ...
        yabasanshiro)
            ...
            ;;
        flycast_fast_umrk)
            grep -Fq -- '-DTARGET_LINUX_ARMv8' "$compile_log" \
                && grep -Fq -- '-D__NEON_OPT' "$compile_log" \
                && grep -Fq -- '-DLOW_END' "$compile_log" \
                && grep -Fq -- 'core/rec-ARM64/rec_arm64.cpp' "$compile_log"
            ;;
    esac || {
...
core_build_lane() {
    ...
    case "$core" in
        ...
        mgba)
            ...
            ;;
        flycast_fast_umrk)
            printf 'custom-flycast-2022-pgo'
            ;;
        *)
...
build_one_core() {
    ...
    case "$core" in
        ...
        flycast)
            ...
            ;;
        flycast_fast_umrk)
            build_flycast_fast_umrk_core && build_status=0
            ;;
        mame)
...
for core in ${requested_cores[@]+"${requested_cores[@]}"}; do
    ...
    if [[ "$build_mode" == "flycast-profile-gen" ]]; then
        if ! build_flycast_fast_umrk_profile_gen; then
            failed_cores+=("$core")
        fi
        continue
    fi
